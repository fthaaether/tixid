...
@section('content')
    <div class="container my-5">
        <form method="POST" action="{{ route('staff.schedules.update', $schedule->id) }}">
            @csrf
            {{-- route method menggunakan patch karena hana akan mengubah sebagian data : price, hours --}}
            @method('PATCH')
            <div class="mb-3">
                <label for="cinema_id" class="col-form-label">Bioskop:</label>
                {{-- dibuatt disabled agar tidak dapat diubah --}}
            ...
            <div class="mb-3">
                ...
                <label for="movie_id" class="col-form-label">Bioskop:</label>
                {{-- dibuatt disabled agar tidak dapat diubah --}}
...
<script>
    function addInput() {
        let content = ` <div class="d-flex align-items-center hour-aditional">
                    ...
                    onclick="this.closest('.hour-aditional').remove()"></i>
                </div>`;
